...
class IntroScene extends Scene {
  ...
  async setupTv() {
    ...
    tv.model.position.set(0, 0, -60); // Start far away on the z-axis
    // video
  ...
  setupSpotLight(target: THREE.Object3D) {
    this.spotLight = new THREE.SpotLight(0xffffff, 2); // Increase intensity
    this.spotLight.position.set(0, 30, -60); // Position the light above the TV
    this.spotLight.target = target; // Point the spotlight towards the TV
  
    // Adjust spotlight properties for better visibility
    this.spotLight.angle = Math.PI / 8; // Narrower beam
    ...
    // Optionally add a helper for visualization
    // const spotLightHelper = new THREE.SpotLightHelper(this.spotLight);
    // this.scene.add(spotLightHelper);
  }

  update(deltaTime: number, audioData: Float32Array | Float32Array[] | number[] | undefined) {
    if (this.tv && this.spotLight) {
      const targetPosition = new THREE.Vector3(0, 0, -10); // Target is the origin
      const currentPosition = this.tv.model.position;
  
      // Linear interpolation to move the TV towards the origin
      const speed = 0.2;
      currentPosition.lerp(targetPosition, speed * deltaTime);
  
      // Update the position of the TV
      this.tv.model.position.set(currentPosition.x, currentPosition.y, currentPosition.z);
  
      // Move spotlight with the TV and update target
      this.spotLight.position.set(currentPosition.x, currentPosition.y + 30, currentPosition.z);
      this.spotLight.target.position.set(currentPosition.x, currentPosition.y, currentPosition.z);
      this.spotLight.target.updateMatrixWorld(); // Ensure target matrix is updated
    }
  }
}
